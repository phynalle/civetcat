...
impl Grammar {
    ...
    fn tokenize_string(&self, text: &str, state: StateCell, tokens: &mut Vec<Token>) -> (StateCell, Option<usize>) {
        let rule: &Rule = state.rule.borrow();
        let matches = rule.find_subpattern(text, &self.rules);
        let matched_rules = matches.into_iter()
            .min_by_key(|m| {
                m.start
            });
        
        let matched_end = state.end_rule
            .borrow()
            .as_ref()
        ...
        let process_rules = move |tokens: &mut Vec<Token>, fr: FindResult, prev_state: StateCell| {
        ...
        fn process_end(tokens: &mut Vec<Token>, m: MatchResult, state: StateCell, text: &str) -> StateCell {
            // capturing 
        ...
        let state = match (matched_rules, matched_end) {
            (Some(m1), Some(m2)) => {
